...
var GnLib = {
    ...
    autoloadContent: function (dataSource, callback) {
        GnLib.loadContent(dataSource, function (template) {
            ...
            for (field in registro) {
                ...
                // Correção do case sensitive para Android
                if( objField.length == 0 ) {
					objField = $("#" + field.toLowerCase());
					if( !registro.extId && registro.extid ) {
						registro.extId = registro.extid;
						registro.prefixImg = registro.prefiximg;
					}
                }
    ...
    applyFilters: function (registros) {
        ...
        for (attr in GnLib.filter) {
            ...
            for (r in registros) {                
                if ( registros[r][attr] ) {
                    if ( typeof(GnLib.filter[attr]) == "string" && registros[r][attr] == GnLib.filter[attr] ) {
                        tmp.push(registros[r]);
                    } else if( typeof(GnLib.filter[attr]) == "object" && GnLib.filter[attr].indexOf(registros[r][attr]) > -1 ) {
                        tmp.push(registros[r]);
                    }                     
                }

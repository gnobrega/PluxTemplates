...
var GnLib = {
	...
	/**
	 * Recupera o arquivo de dados (data.json)
	 */
     ...
     */
    loadContent: function (dataSource, callback, file, playerId) {
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                //Aplica os filtros
                template.registros = GnLib.applyFilters(template.registros, playerId);
     ...
     */
    autoloadContent: function (dataSource, callback, playerId) {
                ...
                if (objField.length > 0) {
                    ...
                    if (objField.prop("tagName") == "IMG" ) {
                        if (value) {
                            ...
                            var src = GnLib.dataSource.substr(0, idx) + "/" + GnLib.prefixImg + value + "." + registro.extId;
                            var objFieldImg
                            objField.hide();
                    ...
                    } else if( objField.prop("tagName") == "VIDEO" ) {
						if (value) {
							var idx = GnLib.dataSource.lastIndexOf("/");
                            var params = value.split("/");
							if( params.length > 4 ) {
								src = GnLib.dataSource.substr(0, idx) + "/" + params[5];
								objField.attr("src", src);
							}
						}
                    } else {
     ...
     */
    applyFilters: function (registros, playerId) {
		var now = new Date();
        ...
        var horaAtual = GnLib.getCurrentTime();
        if (registros.length > 0 ) {
            ...
            for (r in registros) {
				...
				//Filtra pelo Id
				if( playerId && registro.players ) {
					var players = registro.players.split(",");
					if( players.indexOf(playerId) == -1 ) {
						continue;
					}
				} else if( playerId && !registro.players ) {
					continue;
				}
				
				//Filtra pelo dia da semana
				if( registro.dias ) {
					var dias = registro.dias.split(",");
					if( dias.indexOf(now.getDay()+"") == -1 ) {
						continue;
					}
				}
    ...
    /**
     * Gera a hora atual no padrão ISO
     */
	...
	loadPreview: function() {
		...
		console.log(params);
		for( i in params ) {
			if( i.indexOf("branchesId") == -1 ) {
				var field = $("#"+i);
				if( field.length ) {
					field.html(params[i]);

					//Img
					if( i.indexOf("fileId") > -1 ) {
						var src = "/upload/"+db+"/modulos/"+module+"/img_"+params[i]+"."+ext;
